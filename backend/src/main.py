...
# ---------- Measurement ----------
def measurement_phase(ws, joint_system=None, duration_s=30, sampling_rate_est=10.0,
                      out_filename="joint_angles.csv", rezero_on_stillness=True,
                      rezero_window_s=1.5, rezero_gyro_thresh=0.5, rezero_cooldown_s=5.0):
    print("\n=== Measurement Phase ===")
    packets = []
        ...
        if 'IMU1' in pkt and 'IMU2' in pkt:
            ...
            if rezero_on_stillness and len(imu1_window) >= window_n and (time.time() - last_rezero) > rezero_cooldown_s:
                ...
                if g_mean < rezero_gyro_thresh:
                    try:
                        joint_system.reestimate_gyro_bias_from_window(imu1_window, imu2_window)
                        joint_system.set_zero_reference_from_window(imu1_window, imu2_window,
                                                                    gyro_thresh=rezero_gyro_thresh)
    ...
    with open(out_path, 'w') as f:
        ...
        for i, a in enumerate(angles):
            ...
            # 🔴 Stream real-time data point for frontend live plot
            print(f"STREAM_DATA {ts_rel:.3f},{a if a is not None else ''}", flush=True)
